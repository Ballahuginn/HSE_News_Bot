...
def get_vk_post(bot, vk_api):
    ...
    c = threading.Timer(86400, rem_old_vk_posts)
    c.start()
    #rem_old_vk_posts(bot, vk_api)


def rem_old_vk_posts():
    database = sqlite3.connect('HSE_BOT_DB.sqlite')
    db = database.cursor()
    ...
    for p in posts:
        ...
        if p[2] > str(int((datetime.today() - timedelta(hours=24) - datetime.utcfromtimestamp(0)).total_seconds())):
            gmark.append(p[1])
            #print(p[0], p[1], datetime.fromtimestamp(int(p[2])), p[3])

    for g in groups:
        for m in list(set(gmark)):
            if g[0] == m:
                    tfhrs = str(
                    int((datetime.today() - timedelta(hours=24) - datetime.utcfromtimestamp(0)).total_seconds()))
                    print('group ', g, ' is ready to be erased!')
                    db.execute("DELETE FROM Posts WHERE gid = ? AND p_date <= ?", (m, tfhrs, ))
                    database.commit()
    database.close()
